...
func (p *Parser) parseGlobals(packageModel *model.Package, tpkg *types.Package) error {
	...
	for _, name := range scope.Names() {
		...
		case *types.Interface:
			...
			iface.Fullname = types.TypeString(thing.Type(), p.qualifier)
		...
		case *types.Struct:
			...
			structure.Fullname = types.TypeString(thing.Type(), p.qualifier)
...
func (p *Parser) parseVarType(obj types.Object, tpkg *types.Package) (model.Type, error) {
	...
	nameStr := typeStr
	if dotindex := strings.LastIndex(nameStr, "."); dotindex >= 0 {
		nameStr = nameStr[dotindex+1:]
	}
	return model.Type{
		Name:     nameStr,
		Fullname: typeStr,
